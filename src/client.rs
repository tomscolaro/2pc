...
pub struct Client {
    ...
    pub recieve_chan: Receiver<message::ProtocolMessage>,
    pub send_chan: Sender<message::ProtocolMessage>,
    pub done: bool
}

// issue request and send back to coordinator
///
...
impl Client {
    ...
    pub fn new(id_str: String,
               ...
               recieve_chan: Receiver<message::ProtocolMessage>,
               send_chan: Sender<message::ProtocolMessage>
               ) -> Client {
    ...
    pub fn send_next_operation(&mut self) {
        ...
        let mut pm = message::ProtocolMessage::generate(message::MessageType::ClientRequest,
                                                    txid.clone(),
                                                    self.id_str.clone(),
                                                    self.num_requests);
        ...
        if self.num_requests > self.max_requests {
            pm.mtype = message::MessageType::NoMoreRequests;
        }
        
        self.send_chan.send(pm).unwrap();
        ...
        loop {
            match self.recieve_chan.try_recv() {
                Ok(res) => {
                    ...
                    if res.mtype == message::MessageType::ReadyToRecieve{
                        break
                    }

                    if res.mtype == message::MessageType::CoordinatorExit{
                        self.done = true;
                        break
                    }
                },
    ...
    pub fn protocol(&mut self, n_requests: u32) {
        self.max_requests = n_requests.clone();
        //Entry to the coordinator
        self.recv_result();
        // self.send_next_operation();
        // println!("Beginning {:?}, --- {:?}", self.id_str.clone(), self.max_requests);
